...
--1
CREATE PROC SP_1_ListEmployeesBySalary
    @Salary INT
AS SELECT * FROM NHANVIEN
WHERE LUONG > @Salary;

EXEC SP_1_ListEmployeesBySalary 30000

--2
CREATE PROCEDURE SP_2_ListEmployeesByDateRange
    @StartDate DATE,
    @EndDate DATE

AS SELECT NV.MANV, NV.TEN, NV.NGSINH, NV.MA_NQL, SV.TEN AS SUPERVISOR_NAME
FROM NHANVIEN NV
LEFT JOIN NHANVIEN SV ON NV.MA_NQL = SV.MANV
WHERE NV.NGSINH BETWEEN @StartDate AND @EndDate;

EXEC SP_2_ListEmployeesByDateRange '01/09/1955', '07/19/1968' 

-- 3
CREATE PROC SP_3_ListEmployeesByDepartment 
@DepartmentCode INT
AS SELECT * FROM NHANVIEN NV
WHERE NV.MA_NQL = @DepartmentCode;
go

EXEC SP_3_ListEmployeesByDepartment 888665555
go
...
SELECT NV.TEN, NV.MANV, 
	   DATEDIFF(MONTH, NV.NGSINH, GETDATE()) AS WORKING_MONTHS
FROM NHANVIEN NV;
...
CREATE PROC SP_6
AS 
SELECT PB.MAPHG AS DEPARTMENT_CODE, PB.TENPHG AS DEPARTMENT_NAME, 
	   COUNT(NV.MANV) AS EMPLOYEE_COUNT
FROM PHONGBAN PB
...
EXEC SP_9 @city = '450 Trung Vuong, Ha Noi'
